...
public class Q2 {
    public static void main(String[] args) {
     ...
     List<Employee1> employee1= new ArrayList<>();
        ...
        for (Student s:
             bestStudent) {
            employee1.add(new Employee1(s.getRoll(),s.getName(), s.getMarks(), s.getAddress()));
        }
       for (int i=0;i<employee1.size();i++){
           System.out.println(bestStudent.get(i)+"    ->    "+employee1.get(i));
       }
    }
}
/* Write following 2 bean classes:
Create a List of 5 Student object,
Transform the Student whose marks is greater than 500 to the Employee_I object as
roll should be the empId and employee salary should be marks * 10000. by using
Stream API.
 Hint: make use of the map function of Stream API.
Get all the employees in another List and sort all the employees object according to
the salary in descending order by using Lambda Expression.
Print both Employee_I’s List and Original Student’s List using Lambda expression.
*/
